...
export function AusklammernManu() {
  const AufgabenUndLoesungen = [
    ...
    {
      titel: "Klammere nun auch Variabeln aus:",
      aufgabe: "a) a^5 + a^3",
      loesung: "a^3(a^2 + 1)",
    },
    {
    ...
    {
      titel: "Klammere nun doppelt aus:",
            ...
            {AufgabenUndLoesungen.map((item, index) => (
              <View key={index}>
                ...
                <Pressable onPress={() => toggleLoesung(index)}>
                  <MathView math={`\\large ${item.aufgabe}`} style={styles.mathView} resizeMode="cover" />
                </Pressable>
                {selectedLoesungen.includes(index) && (
                  <MathView math={`\\large ${item.loesung}`} style={styles.mathView} resizeMode="cover" />
                )}
              </View>
            ))}
          </View>
